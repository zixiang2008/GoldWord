...
![Logo](GoldWord/goldword-web/www/icons/icon-192x192.png)
...
## 快速开始（Web）
- 本地预览：
  - `cd GoldWord/goldword-web/www && python3 -m http.server 8000` 打开 `http://localhost:8000/`
  - 或直接在浏览器打开 `GoldWord/goldword-web/www/index.html`
- 导入词库：
...
## Android 打包
- 完整步骤与故障排查见 `GoldWord/goldword-web/README-android.md`
- 简要流程：
  - `npm install`
  - `npx cap sync android`
  - Android SDK：已在本地配置 `sdk.dir`
  - 构建签名 APK：`cd GoldWord/goldword-web/android && ./gradlew assembleRelease`
  - 产物：`GoldWord/goldword-web/android/app/build/outputs/apk/release/app-release.apk`
...
## 开发与构建
- 开发：直接编辑 `GoldWord/goldword-web/www/` 下的文件
- 同步到 Android：`npx cap sync android`
- 预览：
  - `http://localhost:8000/` 或直接打开 `GoldWord/goldword-web/www/index.html`
- 重要脚本：
  ...
  build-and-upload:
    ...
    steps:
      ...
      - name: Build web archive
        run: |
          cd GoldWord/goldword-web/www
          zip -r goldword-web.zip .
      - name: Upload Release Assets
        uses: softprops/action-gh-release@v2
        with:
          files: |
            GoldWord/goldword-web/www/goldword-web.zip
        env:
...
- Usage:
  - Local preview: `cd GoldWord/goldword-web/www && python3 -m http.server 8000`
  - Configure GPT under Personal Centre, test connectivity, start learning.
- Deployment: Netlify/Cloudflare Pages/Firebase Hosting; publish `GoldWord/goldword-web/www`.
