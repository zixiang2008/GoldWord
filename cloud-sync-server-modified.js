...
/**
 * GoldWord 云同步服务器 - 修改版
 * Cloud Sync Server - 支持IPv6和IPv4双栈 (端口调整版)
 * 
 * 功能:
 * - 同时监听IPv6和IPv4地址
 * - 自动同步文件到云存储
...
const CONFIG = {
    // 服务器配置
    ports: {
        ipv6: 8001,  // IPv6地址端口 (从8000改为8001)
        ipv4: 8080   // IPv4地址端口 (保持8080)
    },
...
class FileSyncManager {
    ...
    async syncFileToCloud(fileName) {
        ...
        // 这里可以添加实际的云存储上传逻辑
        // 例如: R2, S3, Google Cloud Storage等
    }
...
class RequestHandler {
    ...
    handleFileRequest(req, res, filePath) {
        try {
            ...
            this.logger.info(`文件请求成功: ${filePath}`, {
                size: content.length,
                contentType: contentTypes[ext]
            });
            
        } catch (error) {
            this.logger.error(`处理文件请求失败: ${filePath}`, { error: error.message });
...
class ServerManager {
    ...
    handleRequest(req, res, serverType) {
        ...
        this.logger.info(`收到请求 [${serverType}]`, {
            method: req.method,
            url: req.url,
    ...
    // 启动所有服务器
    async start() {
        this.logger.info('启动GoldWord云同步服务器 (修改版)...');
